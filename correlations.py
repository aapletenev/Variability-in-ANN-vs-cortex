...
if ADD_Poisson:
    Spike_Bern = np.random.poisson(lam=Spike_Bern)
    Spike_Gaus = np.random.poisson(lam=Spike_Gaus)
    Mean_Bern = np.mean(Spike_Bern, axis=1)
    Mean_Gaus = np.mean(Spike_Gaus, axis=1)
else:
...
#Now substitute all image*neuron values with mean  > 100 to NaN
mask_bern = Mean_Bern > 100
mask_gaus = Mean_Gaus > 100
Spike_Bern = np.where(mask_bern[:, np.newaxis, :], np.nan, Spike_Bern)
Spike_Gaus = np.where(mask_gaus[:, np.newaxis, :], np.nan, Spike_Gaus)
...
# Create the 3x3 or 2x2 plot
fig, axes = plt.subplots(nrows=3, ncols=2, figsize=(16, 12))
plt.subplots_adjust(hspace=0.3, wspace=0.3)

plot_hist_comparison(axes[0, 0], cor_z_Bern, cor_z_top_Bern, color='tab:orange', title="Bernoulli Noise", legend_loc= "lower right")
plot_hist_comparison(axes[0, 1], cor_z_Gaus, cor_z_top_Gaus, color='tab:blue', title="Gaussian Noise (σ=10)", legend_loc= "lower right")
...
plt.savefig(wd + '/plots/Anton/noise_correlation' + file_name_add + '.png', dpi=300)
