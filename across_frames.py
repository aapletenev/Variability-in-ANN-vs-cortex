...
Spike_frames_Gaus_blank = get_neurons_of_area(np.load(wd + string_path + 'frames/Gaus_10_blank_10.npy'), labels)
Spike_frames_Gaus_blank[Spike_frames_Gaus_blank > 100] = np.nan
...
Mean_frames_Gaus_blank = np.nanmean(Spike_frames_Gaus_blank, axis=1)  #mean across noise for each image and neuron and frame
Var_frames_Gaus_blank = np.nanvar(Spike_frames_Gaus_blank, axis=1)  #variance across noise for each image and neuron and frame
FF_frames_Gaus_blank = Var_frames_Gaus_blank / (Mean_frames_Gaus_blank + 1e-10)  #fano factor across noise for each image and neuron and frame
...
#Gaussian
slopes_Gaus_blank, r2_Gaus_blank = compute_slope_var_mean(Mean_frames_Gaus_blank, Var_frames_Gaus_blank)
global_res_Gaus_blank = compute_global_frame_fits(Mean_frames_Gaus_blank, Var_frames_Gaus_blank)
a_neurons_Gaus_blank, b_neurons_Gaus_blank, r2_neurons_Gaus_blank = compute_neuron_fits(Mean_frames_Gaus_blank, Var_frames_Gaus_blank, global_res_Gaus_blank)
cor_Gaus_blank = [get_cor_zscored(Spike_frames_Gaus_blank[:,:,i,:])[0] for i in range(Spike_frames_Gaus_blank.shape[2])]
cor_Gaus_blank = np.array(cor_Gaus_blank)


#Delete the a and b for blank as they are poorly defined

a_neurons_Bern_blank[0:10] = np.nan
b_neurons_Bern_blank[0:10] = np.nan
a_neurons_Gaus_blank[0:10] = np.nan
b_neurons_Gaus_blank[0:10] = np.nan

#
fig, axes = plt.subplots(nrows=3, ncols=2, figsize=(10, 12))
# 1. Linear Slope (Fano Factor estimate)
plot_dual_axis_boxplot(axes[0,0], slopes_Bern_blank, slopes_Gaus_blank,
                       title="Fano Factor: Linear Slope of Var-Mean Regression",
                       ylim=0, xline = 9.5)
# 2. Linear Regression R^2
plot_single_axis_boxplot(axes[0,1], r2_Bern_blank, r2_Gaus_blank,
                         title="Variance Explained: Linear Var-Mean Regression",
                         ylabel_suffix="Variance Explained ($R^2$)",
                         legend_loc='best', xline = 9.5)
# 3. Power Law Parameter 'a'
plot_dual_axis_boxplot(axes[1,0], a_neurons_Bern_blank, a_neurons_Gaus_blank,
                       title="Scaling Parameter $a$ (Power Law Fit)",
                       ylabel_suffix="Parameter $a$",
                       ylim=0, xline = 9.5)
# 4. Power Law R^2
plot_single_axis_boxplot(axes[1,1], r2_neurons_Bern_blank, r2_neurons_Gaus_blank,
                         title="Variance Explained: Power Law Fit",
                         ylabel_suffix="Variance Explained ($R^2$)",
                         legend_loc='best', xline = 9.5)
# 5. Power Law Parameter 'b'
plot_single_axis_boxplot(axes[2,0], b_neurons_Bern_blank, b_neurons_Gaus_blank,
                         title="Exponent Parameter $b$ (Power Law Fit)",
                         ylabel_suffix="Parameter $b$",
                         legend_loc='best', xline = 9.5)
# 6. Mean Correlation Coefficient
plot_single_axis_boxplot(axes[2,1], cor_Bern_blank, cor_Gaus_blank,
